...
const Team = ({team,isToday}:TeamProps)=>{
  return (
    <div className="flex flex-col w-1/2 items-center justify-center">
      <img
        className={clsx(
          "w-[90px] h-[60px] border-[4px] border-black shadow-[5px_5px_0_0_black] rounded-[14px]",
          isToday &&
            "w-[180px] h-[120px] border-[5px] shadow-[10px_10px_0_0_black]"
        )}
...
const Versus = ({date}:VersusProps) =>{
  ...
  return <div className='grid relative h-full py-3'>
    ...
    <span className='font-medium text-sm place-self-end bg-black rounded-full px-3 py-1 text-white'>{isFinish ? "Selesai" :moment(date).format("HH:mm")}</span>
  </div>
}
...
export default function Card({match,isToday}:CardProps) {
  return (
    <div
      className={clsx(
        "flex flex-row aspect-video border-[3px] border-black shadow-[8px_8px_0_0_black] hover:shadow-[15px_15px_0_0_black] rounded-[12px] transition-shadow bg-white",
        isToday &&
          "shadow-[15px_15px_0_0_black] hover:shadow-[20px_20px_0_0_black]"
      )}
    >
